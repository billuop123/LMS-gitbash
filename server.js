...
app.put("/books/all/:id", isLoggedin, async (req, res) => {
    ...
    try {
        ...
        if (state.toLowerCase() === 'available' && (checkHistory.rows.length === 0 ||   checkHistory.rows[0].status.toLowerCase() == "returned")) {
        ...
        } else if (state .toLowerCase()=== 'unavailable' && checkHistory.rows.length > 0 && checkHistory.rows[0].book_id == bookId) {
            ...
            if (updatedBook) {
                ...
                const currentUserBookId = await db.query(
                    "SELECT book_id FROM user_history WHERE id = $1",
                    [req.session.user.id]
                );
                ...
                if (currentUserBookId.rows[0].book_id == bookId) {
                    ...
                    updateHistory(req, updatedBook);
                    res.status(200).json({error:"SuccessFully Returned Book"});
                } else {
                    const result = await db.query(
                        "UPDATE books SET status = $1 WHERE id = $2 RETURNING *",
                        ["unavailable", bookId]
                    );
                    res.status(404).json({ error: "You didnot take this book" })
                }
        ...
        } else if (state.toLowerCase() === 'available' && checkHistory.rows.length > 0 && checkHistory.rows[0].status.toLowerCase() == "taken") {
            res.status(403).json({error:"You have already taken a book"})
        } else {
            // Invalid state or user has already taken/returned the book
            res.status(403).json({ error: `Not your book`  });
        }
    } catch (error) {
        console.error(error);
